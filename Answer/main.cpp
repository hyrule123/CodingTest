...
public:
	...
	private:
		Node() : parent{}, left{}, right{}, data{}, color{ RED } {}
		Node(int _data)
			: Node()
		{
			data = _data;
	...
	void Traverse() const;
	const Node* Search(int _data) { return _Search(_data); }
...
{
	if (_node == root) { root->color = BLACK; return; }
...
void RedBlackTree::Delete(Node* _node)
{
	...
	bool isDoubleBlack = (_node->color == BLACK) && (nullptr == replace || replace->color == BLACK);
...
RedBlackTree::Node* RedBlackTree::_Search(int _data)
{
	Node* node = root;
	while (node != nullptr) {
...
int main() {
	...
	for (int i = 0; i < size; ++i) {
		int input; std::cin >> input;
		const RedBlackTree::Node* result = tree.Search(input);
		if (result) {
			std::cout << 1 << '\n';
		}
		else {
